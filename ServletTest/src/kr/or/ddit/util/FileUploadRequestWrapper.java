 ...
 * (HttpServletRequest에 기반한 API를 사용하면서 멀티파트 처리기능이 추가됨)
 */
public class FileUploadRequestWrapper extends HttpServletRequestWrapper{
	...
	public FileUploadRequestWrapper(HttpServletRequest request,
		int memoryThreashold, int fileSizwMax, int maxRequestSize, String repositoryPath) throws FileUploadException {
		
		super(request);
		
		parsing(request, memoryThreashold, fileSizwMax,  maxRequestSize,  repositoryPath );
	 ...
	 */
	private void parsing(HttpServletRequest request, int memoryThreashold, int fileSizeMax,
			int maxRequestSize, String repositoryPath) throws FileUploadException {
		
		if(ServletFileUpload.isMultipartContent(request)) { // 멀티파트인지 구분
			...
			for (int i = 0; i < list.size() ; i++) {
				...
				if(fileItem.isFormField()) { // 폼필드 인경우
					String value = "";
					try {
						value = fileItem.getString("UTF-8"); // 인코딩처리
					} catch (Exception e) {
						e.printStackTrace();
					}
					
					String[] values = (String[]) parameterMap.get(name);
					if (values == null) { // 처음생성하는경우..
