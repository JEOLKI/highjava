...
public class MemberIBatisTest {
	public static void main(String[] args) {
		...
		// bulidpath에 iBatis자르 파일을 셋팅해줘야 한다.
		
		// 1. ibatis의 환경설정파일을 읽어와 실행시킨다.
		try {
			// 1-1. xml문서 읽어오기 // 알고있어야하는 설정 정보들이 xml속에 들어 있는것.
			Charset charset = Charset.forName("UTF-8"); // 설정파일의 인코딩 설정 
			Resources.setCharset(charset); // xml 문서안에 한글이 있기때문에 하는 작업이다. 안해줘도 무방
			...
			//      반환값 : 성공하면 null이 반환된다.
			Object obj = smc.insert("memberTest.insertMember", mv); //useStatementNamespaces="true" 
