...
public class T01_VectorTest {
	public static void main(String[] args) {
		// 여러개의 객체들을 담기위해 Collection 사용 CRUD위해
		...
		// Vector 는 add()메서드를 이용하여 데이터를 추가 할 수 있다.
		v1.add("aaa");
		v1.add(111); // 정수객체로 들어간다 // 객체로 자동으로 바꿔서 넣어준다.
		v1.add(new Integer(123));
		v1.add('a');
		v1.add(true); // 객체로 변환되어 들어간다.
		v1.add(3.14);
		// 자동으로 오토박싱을 통하여 변환후 들어간다.
		
		System.out.println("현재 크기 : " + v1.size());
		
		// Vector는 addElement()메서드를 이용하여 추가 할 수도 있는데
		// 이 메서드는 기본적으로 add()메서드와 같은 기능을 수행함
		// v1.addElement("ccc"); //Vector 타입이면 사용가능하다.
		
		System.out.println("v1 => " + v1.toString());
		
		// add(index, 데이터) => 벡터의 index번째에 '데이터'를 끼워 넣는다. index는 0부터 시작한다.
		// Collection framework가 인텍스를 자동으로 밀어준다.
		
		v1.add(1, "kkk");
		System.out.println("v1 => " + v1.toString());
		
		// set(index, 데이터) => 벡터의 index번째의 값을 주어진 '데이터'로 덮어 쓴다.
		//					=> 반환값 : 원래의 데이터
		...
		// remove(삭제할 데이터) => '삭제할 데이터'를 찾아 삭제하나다.
		//  => 만약 '삭제할 데이터'기  여러개이면 앞에서부터 삭제한다.
		...
		/*
		 제너릭 타입(generic type)
		  => Collection객체를 선언할 때 <>안에 그 Collection이 저장할 데이터 타입을 정해 주는것을 만한다.
		  => 이런식으로 선언하게 되면 그 데이터 타입 이외의 데이터를 저장 할 수 없다.
		     (제너릭 타입으로 선언할 수 있는 데이터 타입은 클래스 이어야 한다.)
		...
		*/
		...
		v2.add("안녕하세요");
		// v2.add(123); //오류 : 다른종류의 데이터를 추가 할 수 없다.
		...
		// 벡터의 데이터들을 순서대로 모두 가져와서 사용하고 싶으면 반복문을 사용하면 된다.(주로 for문을 사용한다.)
		...
		/*
		 ...
		 => 반복 할 때 만다 '배열변수나 Collection계열 변수'의 데이터를 하나씩 꺼내와 '변수명'에 저장한 후 '처리할 내용들'을 처리한다.
		*/
		
		// index값에 접근이 불가능한 단점이 있다.
		...
		//capacity는 담을수 있는 공간 실제로 담은것은 size
		v.ensureCapacity(5); // 현재 용량이 최소용량보다 작다면.... 최소용량 5
							 // 신규용량  = 현재용량 * 2, 그래도 작다면,
							 // 신규용량  = 최소용량으로 설정된다.
